...
function CoverImage({ manga }: { manga: LibraryManga }) {
  ...
  if (manga.coverImage && !imgError) {
    return (
      <img src={manga.coverImage} alt={manga.title} onError={() => setImgError(true)}
...
export default function LibraryPage() {
  ...
  useEffect(() => {
    setLibrary(libraryService.getAll());
    setMounted(true);
  }, []);

  const handleRemove = (mangaId: string) => {
    libraryService.remove(mangaId);
    setLibrary(libraryService.getAll());
  ...
  if (!mounted) return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: 40, height: 40, border: "3px solid rgba(255,107,53,0.2)", borderTop: "3px solid #FF6B35", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
    </div>
  );
    ...
    <>
      <style>{`
        ...
        .manga-card:hover {
          ...
          box-shadow: 0 12px 40px rgba(0,0,0,0.4), 0 0 20px rgba(255,107,53,0.08);
        }
          ...
          <div style={{ position: "relative", overflow: "hidden", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
            ...
            <div style={{ position: "relative", maxWidth: 1280, margin: "0 auto", padding: "clamp(5rem,10vw,7rem) 1.5rem 0" }}>
              ...
              <nav className="fade-up" style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: "1.5rem" }}>
                ...
                <Link href="/dashboard" style={{ color: "rgba(255,255,255,0.3)", textDecoration: "none", fontFamily: "'Share Tech Mono', monospace", fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", transition: "color 0.2s" }}
              ...
              <div className="fade-up" style={{ animationDelay: "0.1s", paddingBottom: "2rem" }}>
                <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(2.5rem,7vw,5rem)", letterSpacing: "0.03em", lineHeight: 0.9, marginBottom: "0.5rem", background: "linear-gradient(135deg, #fff 0%, rgba(255,255,255,0.8) 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
                  My Library
                </h1>
          ...
          <div style={{ maxWidth: 1280, margin: "1.25rem auto 0", padding: "0 1.5rem" }}>
            ...
            {library.length === 0 && (
              <div style={{ textAlign: "center", padding: "6rem 2rem", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 24 }}>
                ...
                <Link href="/manga" style={{ textDecoration: "none" }}>
                  <button style={{ padding: "12px 28px", background: "linear-gradient(135deg, #FF6B35, #E63946)", border: "none", borderRadius: 14, color: "#fff", fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.1rem", letterSpacing: "0.1em", cursor: "pointer", boxShadow: "0 0 20px rgba(255,107,53,0.3)" }}>
                    Browse Manga
                  </button>
                </Link>
              </div>
            ...
            {library.length > 0 && filtered.length === 0 && (
              <div style={{ textAlign: "center", padding: "4rem 2rem", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 24 }}>
                ...
                <button onClick={() => { setSearch(""); setFilterStatus("All"); }} style={{ marginTop: 12, background: "none", border: "none", color: "#FF6B35", fontFamily: "'Share Tech Mono', monospace", fontSize: 12, letterSpacing: "0.1em", cursor: "pointer", textDecoration: "underline" }}>
                  Clear Filters
                </button>
              </div>
            )}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 16 }}>
                {filtered.map((manga, i) => (
                  <div key={manga._id} className="manga-card" style={{ animationDelay: `${i * 0.04}s` }}>
                    {/* Cover */}
                    <Link href={`/manga/${manga._id}`} style={{ textDecoration: "none", display: "block" }}>
                      <div style={{ aspectRatio: "3/4", overflow: "hidden", position: "relative" }}>
                        <CoverImage manga={manga} />
                        {/* Remove button (revealed on hover) */}
                        <button
                          className="remove-btn"
                          onClick={e => { e.preventDefault(); e.stopPropagation(); setRemoveConfirm(manga._id); }}
                        ...
                        <div style={{ position: "absolute", bottom: 8, left: 8 }}>
                          <span style={{ ...statusStyle(manga.status), fontFamily: "'Share Tech Mono', monospace", fontSize: 8, letterSpacing: "0.1em", textTransform: "uppercase", padding: "2px 7px", borderRadius: 10 }}>
                            {manga.status}
                          </span>
                        </div>
                      </div>
                      <div style={{ padding: "12px 12px 14px" }}>
                        <p className="card-title" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "1rem", letterSpacing: "0.04em", color: "rgba(255,255,255,0.9)", lineHeight: 1.15, marginBottom: 4, transition: "color 0.2s", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                          {manga.title}
                        </p>
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          {manga.rating > 0 && (
                            <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "0.85rem", color: "#F4D03F", letterSpacing: "0.04em" }}>★ {manga.rating.toFixed(1)}</span>
                          )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {filtered.map((manga) => (
                  <div key={manga._id} className="list-row">
                    {/* Cover thumbnail */}
                    <Link href={`/manga/${manga._id}`} style={{ textDecoration: "none", flexShrink: 0 }}>
                      <div style={{ width: 48, height: 64, borderRadius: 10, overflow: "hidden", border: "1px solid rgba(255,107,53,0.15)" }}>
                        <CoverImage manga={manga} />
                      </div>
                    </Link>

                    {/* Info */}
                    <Link href={`/manga/${manga._id}`} style={{ textDecoration: "none", flex: 1, minWidth: 0 }}>
                      <p className="list-title" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.1rem", letterSpacing: "0.04em", color: "rgba(255,255,255,0.9)", lineHeight: 1.15, marginBottom: 3, transition: "color 0.2s", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {manga.title}
                      </p>
                    ...
                    {/* Rating */}
                    <div style={{ textAlign: "center", flexShrink: 0, display: "none", minWidth: 60 }} className="list-rating">
                      {manga.rating > 0 && (
                        <>
                          <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.25rem", color: "#F4D03F", lineHeight: 1 }}>★ {manga.rating.toFixed(1)}</p>
                          <p style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 8, color: "rgba(255,255,255,0.2)", letterSpacing: "0.06em" }}>/ 10</p>
                        </>
                      )}
                    </div>
                    ...
                    {/* Remove */}
                    <button className="remove-btn-list" onClick={() => setRemoveConfirm(manga._id)}>✕ Remove</button>
                  </div>
                ))}
              </div>
            )}
      ...
      {removeConfirm && (() => {
        const m = library.find(x => x._id === removeConfirm);
        return (
            ...
            <div style={{ background: "#111118", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: "2rem", maxWidth: 380, width: "100%", boxShadow: "0 0 60px rgba(0,0,0,0.8)" }}>
              ...
              {m && (
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: "1.5rem", padding: "14px 16px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14 }}>
                  <div style={{ width: 44, height: 58, borderRadius: 8, overflow: "hidden", flexShrink: 0, border: "1px solid rgba(255,107,53,0.15)" }}>
                    <CoverImage manga={m} />
                  </div>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => handleRemove(removeConfirm)} style={{ flex: 1, padding: "12px", background: "linear-gradient(135deg, #ef4444, #dc2626)", border: "none", borderRadius: 12, color: "#fff", fontFamily: "'Bebas Neue', sans-serif", fontSize: "1rem", letterSpacing: "0.1em", cursor: "pointer" }}>Remove</button>
