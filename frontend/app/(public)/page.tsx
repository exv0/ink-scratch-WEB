...
// id is used to compute a deterministic duration — avoids SSR/client hydration mismatch
function InkParticle({ id, delay, x, size }: { id: number; delay: number; x: number; size: number }) {
  return (
    <div
      ...
      style={{
        ...
        animationDuration: `${6 + (id % 5) * 0.8}s`,
        filter: "blur(1px)",
      }}
    />
  );
...
function FeatureCard({
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-full h-[1px] bg-linear-to-r from-transparent via-orange-500/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
...
// ─── Genre badge — clickable Link to browse page ──────────────────────────────
function GenreBadge({ label, active }: { label: string; active?: boolean }) {
  return (
    <Link
      href={`/browse?genre=${encodeURIComponent(label)}`}
      className={`px-3 py-1 rounded-full text-xs font-bold border font-mono uppercase tracking-wider transition-all duration-200 hover:scale-105 ${
        active
          ? "bg-orange-500/20 border-orange-500/50 text-orange-400"
          : "bg-white/5 border-white/10 text-white/40 hover:border-orange-500/40 hover:text-orange-400 hover:bg-orange-500/10"
      }`}
    >
      {label}
    </Link>
...
export default function HomePage() {
  ...
  // All particle data is fully deterministic — no Math.random()
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .glitch-layer   { animation: glitch 5s infinite;       color: #FF6B35; text-shadow: -2px 0 #E63946; }
        .glitch-layer-2 { animation: glitch 5s infinite 0.1s;  color: #E63946; text-shadow:  2px 0 #FF6B35; }
        ...
        .hero-text-stroke { -webkit-text-stroke: 2px rgba(255,107,53,0.4); color: transparent; }
        ...
        .glow-btn       { box-shadow: 0 0 20px rgba(255,107,53,0.3), 0 0 60px rgba(255,107,53,0.1), inset 0 1px 0 rgba(255,255,255,0.1); }
        .glow-btn:hover { box-shadow: 0 0 30px rgba(255,107,53,0.5), 0 0 80px rgba(255,107,53,0.2), inset 0 1px 0 rgba(255,255,255,0.15); }
      `}</style>
      ...
      <div className="relative z-10 min-h-screen text-white font-jp">
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start fade-up" style={{ animationDelay: "0.6s" }}>
              <Link href={ctaPrimaryHref}>
                {/* Tailwind v4: bg-linear-to-r */}
                <button className="relative group px-8 py-4 bg-linear-to-r from-orange-500 to-red-500 text-white font-black text-lg rounded-xl glow-btn transition-all duration-300 hover:scale-105 overflow-hidden manga-panel">
                  <span className="relative z-10 font-display tracking-wider">{ctaPrimaryLabel}</span>
                  <div className="absolute inset-0 bg-linear-to-r from-red-500 to-orange-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            ...
            <div className="border border-orange-500/30 rounded-lg p-4 space-y-3" style={{ background: "rgba(255,107,53,0.03)" }}>
              {[70, 45, 85, 30, 60].map((w, i) => (
                <div key={i} className="flex items-center gap-2">
                  {/* Tailwind v4: bg-linear-to-r */}
                  <div className="h-2 rounded-full bg-linear-to-r from-orange-500/60 to-transparent" style={{ width: `${w}%` }} />
                </div>
              ))}
              {/* Tailwind v4: bg-linear-to-br */}
              <div className="w-full h-24 rounded bg-linear-to-br from-orange-500/10 to-red-500/10 border border-orange-500/20 mt-4 flex items-center justify-center">
        ...
        <section className="relative py-24 px-6">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <FeatureCard icon="⚡" title="Instant Access"   description="Start reading in seconds. No downloads, no installs — just click and dive into the story."                         accent="#FF6B35" delay="0s"   />
              <FeatureCard icon="☁️" title="Cloud Sync"       description="Your reading progress follows you everywhere. Pick up exactly where you left off on any device."                  accent="#E63946" delay="0.1s" />
              <FeatureCard icon="📱" title="Anywhere"         description="Phone, tablet, laptop — the reader adapts perfectly to every screen size."                                        accent="#FF9A56" delay="0.2s" />
              <FeatureCard icon="🔖" title="Smart Bookmarks"  description="Save favorite panels, chapters, and series with a single tap."                                                    accent="#FF6B35" delay="0.3s" />
              <FeatureCard icon="🌙" title="Dark & Light"     description="Switch between themes tied to your account — reads great day or night."                                           accent="#E63946" delay="0.4s" />
              <FeatureCard icon="🔍" title="Powerful Search"  description="Find manga by genre, author, rating, or status. Discover your next obsession."                                    accent="#FF6B35" delay="0.5s" />
            </div>
          </div>
        </section>
            ...
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                <Link
                  ...
                  className="px-4 py-2 rounded-lg border border-white/10 bg-white/[0.03] text-white/50 text-sm font-mono-custom uppercase tracking-wider hover:border-orange-500/50 hover:text-orange-400 hover:bg-orange-500/10 hover:scale-105 transition-all duration-200"
                >
                  {g}
                </Link>
              ))}
        ...
        <section className="relative py-32 px-6 overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden">
            <span className="font-display text-[200px] md:text-[300px] opacity-[0.03] text-white select-none leading-none">READ</span>
          </div>

          <div className="relative max-w-4xl mx-auto text-center">
            <div className="flex items-center justify-center gap-3 mb-8">
              {/* Tailwind v4: bg-linear-to-r / bg-linear-to-l */}
              <div className="flex-1 h-[1px] bg-linear-to-r from-transparent to-orange-500/30" />
              <span className="chapter-tag text-orange-500/70">Final Arc</span>
              <div className="flex-1 h-[1px] bg-linear-to-l from-transparent to-orange-500/30" />
            </div>
            ...
            <Link href={ctaPrimaryHref}>
              {/* Tailwind v4: bg-linear-to-r */}
              <button className="relative group px-12 py-5 bg-linear-to-r from-orange-500 to-red-500 text-white font-black text-xl rounded-xl glow-btn transition-all duration-300 hover:scale-105 overflow-hidden">
        ...
        <footer className="relative border-t border-white/5 py-10 px-6">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              {/* Tailwind v4: bg-linear-to-br */}
              <div className="w-6 h-6 rounded-md bg-linear-to-br from-orange-500 to-red-500 flex items-center justify-center">
